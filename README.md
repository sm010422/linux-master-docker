...
![Rocky Linux](https://img.shields.io/badge/Rocky_Linux-8-10B981?logo=rockylinux&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-Compose-2496ED?logo=docker&logoColor=white)
![systemd](https://img.shields.io/badge/systemd-enabled-lightgrey)
![License](https://img.shields.io/badge/license-MIT-blue)

Rocky Linux 8 (RHEL 계열) + systemd 기반으로 실제 시험 환경과 최대한 유사하게 구성했습니다.  
...
> **주의**: `docker compose up` 을 직접 사용하지 마세요.  
> Docker Compose v5가 `cgroupns_mode` 속성을 거부하기 때문에 systemd가 정상 동작하지 않습니다.  
> 반드시 아래 `make` 명령어를 사용하세요.

### 시작 / 종료

```bash
# 최초 빌드 (이미지 빌드, 5~10분 소요)
make build

# 컨테이너 시작 (빌드 후 또는 재시작 시)
make up
...
# 완전 초기화 (볼륨·이미지·네트워크 모두 삭제, 연습 데이터 삭제됨)
make clean
```
...
```bash
# ── 방법 1: SSH 접속 (실제 서버처럼) ──────────────────
make ssh1     # server1  (비밀번호: toor)
make ssh2     # server2  (비밀번호: toor)
...
make exec1    # server1 bash
make exec2    # server2 bash
...
- `make down`은 볼륨을 삭제하지 않아 연습 데이터가 유지됩니다. 완전 초기화는 `make clean` 을 사용하세요.
- `docker compose up` 을 직접 실행하면 systemd가 cgroup을 생성하지 못해 SSH 접속이 불가능합니다. 반드시 `make up` 을 사용하세요.
